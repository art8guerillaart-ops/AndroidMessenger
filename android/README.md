...
Нативный Android-клиент (Kotlin + Jetpack Compose) к FastAPI-бэкенду
(`../backend/main.py` в этом же репозитории, тот же, что использует веб-версия).
Дизайн — тот же визуальный язык:
...
1. Открой Android Studio → **Open** → выбери папку `android/` — это подпапка
   репозитория (соседняя с `backend/`), не корень репозитория.
...
Сам бэкенд — рядом, в `../backend/` (`backend/main.py` от корня репозитория,
см. `backend/.env.example` для настройки SMTP).
...
не настроен в `../backend/.env` — точно как в веб-версии.
