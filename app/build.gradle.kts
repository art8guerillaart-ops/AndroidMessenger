...
android {
    ...
    defaultConfig {
        ...
        // LAN IP хоста в локальной сети — работает и с эмулятором (через виртуальный
        // роутер), и с реальными устройствами в той же Wi-Fi сети. Сервер должен
        // слушать 0.0.0.0, а не 127.0.0.1 (см. запуск uvicorn --host 0.0.0.0).
        // Если сервер задеплоен (например, на Render/VPS) — подставь его https-адрес.
        buildConfigField("String", "BASE_URL", "\"http://192.168.0.102:8000\"")
    }
